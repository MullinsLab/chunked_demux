	...
	elseif index_type == "Index_primer"
    	...
    	for template in collect(keys(samples))
        	...
        	if (indexes["fwd_index"],indexes["rev_index"]) in index_tuples
           		...
           		#for demux only we don't want to trim forward and reverse primer sequences
           		index_trimmed = [double_primer_trim(s,p,Index_F_univ,Index_R_univ) for (s,p,n) in template_seqs];
				#index_trimmed = [double_primer_trim(s,p,Index_F_univ,Index_R_univ*samples[template]["rev_primer"]) for (s,p,n) in template_seqs];
            	#match template with forward primer not needed for demux only
            	...
            	if length(index_trimmed) == 0
                	@warn "No reads found for $(template)";
                	template_counts[template] = 0
                	continue
            	end

            	filtered_seqs = [s for (s,p) in index_trimmed]
            	filtered_phreds = [p for (s,p) in index_trimmed]
            	filtered_names = names[[i[3] for i in template_seqs]]
